...
{          
	...
	HRESULT test::InitDevice(HWND _hwnd)
	{
		...
		if (g_RenderManager->CreateDevices(width, height, m_hwnd, driverTypes, numDriverTypes, createDeviceFlags, featureLevels, numFeatureLevels, &g_featureLevel))
			return E_FAIL;
...
#elif defined(OGL)
		glfwGetFramebufferSize(g_RenderManager->getOGLWindow(), &width, &height);
#endif
	...
	void test::Clear()
	{
		...
		/*Set shaders and send respective data*/
		...
		g_RenderManager->VSSetConstantBuffers(3, 1, g_RenderManager->GetLightDirectionBuffer());
		...
		/*Clear screen and set the render target*/
...
#elif defined(OGL)
		glfwSwapBuffers(g_RenderManager->getOGLWindow());
#endif
	}
